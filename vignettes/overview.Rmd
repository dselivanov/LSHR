...
Intuition is very straighforward. Let's look down the permuted columns $c_1$ and $c_2$ until we detect **1**.  
  ...
  * In case (0, 1) or (1, 0) $h(c_1) \neq h(c_2)$. So the probability over all permutations of non-zero rows that $h(c_1) = h(c_2)$ is the same as $J(c_1, c_2)$.  
...
# Implementation
Aim of this package is to be fast and memory efficient. We use [Rcpp](https://github.com/RcppCore/Rcpp), [fastmatch](https://github.com/s-u/fastmatch) and [data.table](github.com/Rdatatable/data.table/) packages.
